...
for count in 1...perfis.row_count               # Percorrer todos os perfis da planilha
    ...
    for count in 1...direitos.row_count         # Percorrer todos os direitos da planilha
		...
		file << "cucumber --tag @#{direito} Perfil=#{perfil} --format html --out=\"#{report_name}\" TEST_ENV=#{ambiente} RELEASE_ALM=\"[HML] Homologação Ciclo 1\" CICLO_ALM=\"[Automação] Prevenção e Segurança\" TESTSET_ALM=\"#{testset}\" EVIDENCIA_ALM=S CAMINHO_LOG_EXECUCAO=\"#{nome_arquivo_log}\" SOBRESCREVE_REGISTRO_LOG=S \n"
        file << "cd #{dirname}\n"
        file.close
